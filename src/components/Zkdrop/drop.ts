...
export async function usdcToEth(
  ...
  const zeroXPrice = await fetchZeroXPrice(
    amount.toString(),
    TOKENS_L1_LIST.USDC,
    TOKENS_L1_LIST.WETH
  );

  if (!zeroXPrice.buyAmount) {
    throw Error('No buy amount from 0x');
  }

  const amountOutMin = ethers.BigNumber.from(zeroXPrice.buyAmount)
    .mul(90)
    .div(100);
